...
const int maxn=100005;
const int maxx=1001;
int value[maxx],number[maxx];
bool dp[maxn];
int kind,total;
int ans=0;

void completePack(int cost){  //　　转换成二进制　　　可以降低时间复杂度　Ｏ(n*lognumber[i]);
...
void multiplyPack(int cost,int weight,int number){
	if(cost*number>=total)completePack(cost);    //如果cost*number>=total 那么就可以把　多重背包当做完全背包处理
	else {                                              //所以出现了　调用完全背包的函数
		int k=1;
		while(k<number){
			zeroOnePack(k*cost);              //否这用二进制的思想　　把多个物品用　０１背包来处理
			number-=k;
			k<<=1;
		}
		zeroOnePack(number*cost);   //这里是处理最后剩下的　　物品
	}
}
int main(){
	while(cin>>kind>>total&&kind&&total){
		...
		ans=0;
		dp[0]=1;
		for(int i=1;i<=kind;i++)scanf("%d",&value[i]);
		for(int i=1;i<=kind;i++)scanf("%d",&number[i]);

		for(int i=1;i<=kind;i++)
			if(number[i])multiplyPack(value[i],value[i],number[i]);    //这里相当于０１背包的第一层循环
		for(int i=1;i<=total;i++)if(dp[i])ans++;
		printf("%d\n",ans);
	}
	return 0;
}