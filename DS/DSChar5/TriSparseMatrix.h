...
public:
	...
	// 赋值运算符重载
	TriSparseMatrix<ElemType> operator +(const TriSparseMatrix<ElemType> &copy);
	void  setadd(int r, int c, const ElemType &v);           //为重载    +   做铺垫
...
template <class ElemType>
void  TriSparseMatrix<ElemType>::setadd(int r, int c, const ElemType &v)

{
	if (num == 0){
		...
		int i, j;								// 工作变量
		for (j = num - 1; j >= 0 && (r < triElems[j].row || (r == triElems[j].row && c < triElems[j].col)); j--);   // 查找三元组位置
...
TriSparseMatrix<ElemType> TriSparseMatrix<ElemType>::operator +(const TriSparseMatrix<ElemType> &copy)
{
	...
	for (int i = 0; i<num; i++)temp.setadd(triElems[i].row, triElems[i].col, triElems[i].value);
	for (int j = 0; j<copy.GetNum(); j++)temp.setadd(copy.triElems[j].row, copy.triElems[j].col, copy.triElems[j].value);
